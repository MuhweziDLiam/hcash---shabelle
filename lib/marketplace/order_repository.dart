import 'package:h_cash/marketplace/order_detail_response.dart';
import 'package:h_cash/marketplace/order_item_response.dart';
import 'package:h_cash/marketplace/order_mini_response.dart';
...
class OrderRepository {
  Future<OrderMiniResponse> getOrderList(
    ...
    Uri url = Uri.parse("${shabelleAPI}/purchase-history" +
  ...
  Future<OrderDetailResponse> getOrderDetails({@required int id = 0}) async {
    Uri url =
        Uri.parse("${shabelleAPI}/purchase-history-details/" + id.toString());
  ...
  Future<OrderItemResponse> getOrderItems({@required int id = 0}) async {
    Uri url =
        Uri.parse("${shabelleAPI}/purchase-history-items/" + id.toString());
    final response = await http.get(url, headers: {
      "Authorization": "Bearer ${access_token.$}",
      "App-Language": app_language.$,
    });
